  ...
  while (it != NULL)
  {
    ...
    if (partition != NULL)
    {
      ...
      else if (partition->subtype == ESP_PARTITION_SUBTYPE_DATA_FAT)
      {
        ...
        if (strcmp(partition->label, "vfs") == 0)
          MAX_FAT_vfs = partition->size;
        else if (strcmp(partition->label, "sys") == 0)
          MAX_FAT_sys = partition->size;
      }
    }
    it = esp_partition_next(it);
  }
  esp_partition_iterator_release(it);
  if (MAX_SPIFFS == 0 && askSpiffs)
...
// Init Display
  ...
  LongPress = true;
  while (millis() < i + 5000)
  {                // increased from 2500 to 5000
    initDisplay(); // Inicia o display

    if (millis() > (i + j * 500))
    { // Serial message each ~500ms
...
void loop()
{
  ...
  getBrightness();
  if (!sdcardMounted)
    index = 1; // if SD card is not present, paint SD square grey and auto select OTA
...
void loop()
{
  ...
  { // will try to connect to a saved network
    for (int i = 0; i < nets; i++)
    {
      Serial.println("Target: " + ssid + " Network: " + WiFi.SSID(i));
      if (ssid == WiFi.SSID(i))
